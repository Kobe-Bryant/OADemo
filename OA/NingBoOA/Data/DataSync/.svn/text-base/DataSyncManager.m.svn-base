...
#define kFirstDownloadUserDefaultsTag @"hasDownloadContactData"

@implementation DataSyncManager
@synthesize syncRequest,networkQueue,pregress,aryHandlers;
...
-(void) allSyncFinished :(ASINetworkQueue *)queue{
    ...
    for(NSString *table in aryTables)
    {
        ...
        if([key length] > 0)
        {
            [dbHelper deleteDupRecords:[NSString stringWithFormat:@"%@_TEMP", table] byKeyColumn:key];
        }
    }
    //将临时表数据转移
    for(NSString *table in aryTables)
    ...
    [[NSUserDefaults standardUserDefaults] setObject:currentSyncTime forKey:@"ContactsLastSyncTime"];
...
-(BOOL)syncAllTables:(BOOL)now;{
    ...
    if(now == NO){
        //获取上一次同步通讯录的时间,如果上一次同步的时间和当前时间间隔比较如果大于7天，立即同步数据
        NSString *contactsLastSyncTime = [[NSUserDefaults standardUserDefaults] stringForKey:@"ContactsLastSyncTime"];
        if(contactsLastSyncTime.length > 0)
        {
            NSDateFormatter *df = [[NSDateFormatter alloc] init];
            [df setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
            NSDate *date = [df dateFromString:contactsLastSyncTime];
