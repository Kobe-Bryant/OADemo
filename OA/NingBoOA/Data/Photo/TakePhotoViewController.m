...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *seeItem = [UICustomBarButtonItem woodBarButtonItemWithText:@"上传"];
    ...
    [seeButton addTarget:self action:@selector(uploadButtonClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)makeView
{
    ...
    for(int i = 0; i < count; i++)
    {
        ...
        if(i != count-1)
        {
            ...
            photoName.borderStyle = UITextBorderStyleLine;
...
//                              otherButtonTitles:@"相机拍摄照片",@"从图片库获取图片",nil];
//    [mySheet showInView:self.view];
    ...
    self.photoPopoverController.popoverContentSize = CGSizeMake(320, 480);
    //CGRect rect = CGRectMake(110, 220, 320, 400);
    [self.photoPopoverController presentPopoverFromRect:sender.bounds inView:sender permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
...
//    if ([self.checkedPhotoList count] >= 1 && ![self.checkedPhotoList containsObject:[NSNumber numberWithInt:index]]) {
//        UIAlertView *alertView  = [[UIAlertView alloc] initWithTitle:@"提示" message:@"只允许选择一张图片上传!" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
//        [alertView show];
//        return;
//    }
    ...
    else
    {
        ...
        [layer setCornerRadius:0];
        [layer setBorderWidth:5];
        UIColor *blueColor = [UIColor colorWithRed:214.f/255.f green:234.f/255.f blue:254.f/255 alpha:1.0];
        [layer setBorderColor:[blueColor CGColor]];
        //添加到待上传列表
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    NSData *compressData  = UIImagePNGRepresentation(originImage);
    
    CGFloat compress =  (1024*600*1.0)/[compressData length]; //
    if(compress > 1) compress = 1.0;
    
    NSData *imgData   = UIImageJPEGRepresentation(originImage,compress);
    ...
    BOOL ret = [fileManager createFileAtPath:[filePath stringByAppendingPathComponent:fileName] contents:imgData attributes:nil];
    if(ret)
    {
