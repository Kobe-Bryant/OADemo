...
@interface FileOutManagerController ()
@property (nonatomic,strong) UIPopoverController *popController;
@end

@implementation FileOutManagerController
...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *listItem = [UICustomBarButtonItem woodBarButtonItemWithText:@"发文管理"];
    ...
    self.outListParser.serviceName = @"HANDLEPOSTDOCUMENT";
...
- (void)returnSelectResult:(NSString *)type selected:(NSInteger)row
{
    ...
    switch (row) {
        case 0:
            ...
            self.title = @"在办→按缓急";
            self.outListParser = [[OutHandlingParser alloc] init];
            self.outListParser.type = @"urgent";
            self.outListParser.serviceName = @"HANDLEPOSTDOCUMENT";
            self.outListParser.showView = self.view;
            self.outListParser.delegate = self;
            [self.outListParser requestData];
        ...
        case 3:
            ...
            self.outListParser.type = @"date";
            self.outListParser.serviceName = @"HANDLEPOSTEDDOCUMENT";
            self.outListParser.showView = self.view;
            self.outListParser.delegate = self;
            [self.outListParser requestData];
            break;
        ...
        case 5:
            ...
            self.outListParser.serviceName = @"PostDocumentBackPercent";
            self.outListParser.showView = self.view;
            [self.outListParser requestData];
            break;
        ...
        case 6:
            ...
            self.outListParser.serviceName = @"InquiryPostDocument";
            self.outListParser.showView = self.view;
            self.outListParser.delegate = self;
            break;
...
- (void)didSelectFileOutListItem:(NSString *)docid process:(NSString *)name{
    ...
    self.webServiceHelper = [[WebServiceHelper alloc] initWithUrl:strUrl method:@"GetFileOutDocumentInfo" nameSpace:WEBSERVICE_NAMESPACE  parameters:params delegate:self];
...
- (void)loadFileOutDetailViewProcess:(NSString *)name competence:(NSString *)status{
    FileOutDetailViewController *fileOutDetailViewController = [[FileOutDetailViewController alloc] initWithNibName:@"FileOutDetailViewController" bundle:nil];
    fileOutDetailViewController.docid = self.docid;
    fileOutDetailViewController.process = self.processName;
    fileOutDetailViewController.infoDict = self.postDocInfo;
    fileOutDetailViewController.delegate = self;
    fileOutDetailViewController.responder = self;
    fileOutDetailViewController.competence = status;
    [self.navigationController pushViewController:fileOutDetailViewController animated:YES];
...
- (void)docManagerList:(id)sender
{
    ...
    fileManagerListController.fileOutList = @[@"在办→按缓急",@"在办→按日期",@"在办→按环节",@"已办→按日期",@"已办→按文号",@"退回率",@"查询"];
    ...
    popController.popoverContentSize = CGSizeMake(240,44*7);
...
- (void)processWebData:(NSData *)webData
{
    ...
    WebDataParserHelper *webDataHelper = [[WebDataParserHelper alloc] initWithFieldName:@"GetFileOutDocumentInfoReturn" andWithJSONDelegate:self];
