...
@interface FileInManagerController ()
@property (nonatomic,strong) UIPopoverController *popController;
@end

@implementation FileInManagerController
...
- (void)viewDidLoad
{
   ...
   self.inListParser.type = @"urgent";
    self.inListParser.delegate = self;
    self.inListParser.serviceName = @"HandleReceiveDocument";
...
- (void)returnSelectResult:(NSString *)type selected:(NSInteger)row
{
    ...
    switch (row) {
        ...
        case 3:
            ...
            self.inListParser.serviceName = @"HaveDoneReceiveDocument";
            self.inListParser.delegate = self;
            self.inListParser.showView = self.view;
            [self.inListParser requestData];
            break;
        ...
        case 4:
            ...
            self.inListParser.serviceName = @"AllReceiveDocument";
            self.inListParser.delegate = self;
            self.inListParser.showView = self.view;
            [self.inListParser requestData];
            break;
        ...
        case 5:
            ...
            self.inListParser.serviceName = @"ToDateReceiveDocument";
            self.inListParser.showView = self.view;
            self.inListParser.delegate = self;
            [self.inListParser requestData];
            break;
        ...
        case 6:
            ...
            self.inListParser.serviceName = @"InquiryReceiveDocument";
            self.inListParser.showView = self.view;
            self.inListParser.delegate = self;
            isShow = YES;
            break;
...
- (void)didSelectFileInListItem:(NSString *)docid process:(NSString *)name{
    ...
    self.webServiceHelper = [[WebServiceHelper alloc] initWithUrl:strUrl method:@"GetFileInDocumentInfo" nameSpace:WEBSERVICE_NAMESPACE  parameters:params delegate:self];
...
- (void)loadFileInDetailViewProcess:(NSString *)name competence:(NSString *)status {
    FileInDetailViewController *fileInDetailViewController = [[FileInDetailViewController alloc] initWithNibName:@"FileInDetailViewController" bundle:nil];
...
- (void)docManagerList:(id)sender
{
    ...
    fileManagerListController.fileOutList = @[@"在办→按缓急",@"在办→按日期",@"在办→按环节",@"已办收文",@"所有收文",@"期限收文",@"查询"];
...
- (void)processWebData:(NSData *)webData
{
    ...
    WebDataParserHelper *webDataHelper = [[WebDataParserHelper alloc] initWithFieldName:@"GetFileInDocumentInfoReturn" andWithJSONDelegate:self];
