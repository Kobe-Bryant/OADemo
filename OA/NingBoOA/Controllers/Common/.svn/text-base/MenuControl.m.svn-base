...
-(id)initWithFrame:(CGRect)frame andMenuInfo:(NSDictionary*)info{
    ...
    if (self) {
        ...
        [self.badge setFrame:CGRectMake(frame.size.width-40, -badge.frame.size.height*1/3, badge.frame.size.width, badge.frame.size.height)];
...
-(void)showIconBadge:(NSString *)valueBadge{
    NSInteger number = [valueBadge integerValue];
    if([valueBadge length] <= 0 || [valueBadge isEqualToString:@"0"] || number <= 0)
        badge.hidden = YES;
    else{
        badge.hidden = NO;
