...
@interface ProjectRunDetailViewController ()

@property (strong, nonatomic) UIPopoverController *popController;

@end

@implementation ProjectRunDetailViewController
...
- (void)setControlEdit{
    ...
    [scrollView setContentSize:CGSizeMake(768, 1680)];
    
    self.sqbgTxtView.layer.borderColor = [UIColor colorWithRed:229.f/255.f green:229.f/255.f blue:229.f/255.f alpha:1.0].CGColor;
    self.sqbgTxtView.layer.borderWidth =1.5;
    self.sqbgTxtView.layer.cornerRadius =6.0;
...
- (void)loadApplyInfo{
    self.xmjsmc.text = [self.infoDict objectForKey:@"Hy_Xmjsmc"];
    self.jsdd.text   = [self.infoDict objectForKey:@"Hy_Jsdd"];
    self.ssqy.text   = [self.infoDict objectForKey:@"Hy_Ssqy"];
    self.xmztz.text  = [self.infoDict objectForKey:@"Hy_Xmztz"];
    self.syxsqmc.text = [self.infoDict objectForKey:@"Hy_Syxsqmc"];
    self.syxsqrq.text = [self.infoDict objectForKey:@"Hy_Syxsqrq"];
    self.trsyxrq.text = [self.infoDict objectForKey:@"Hy_Trsyxrq"];
}
...
-(void)sureContinue
{
    ...
    continueProcessViewController.serviceName = @"testRunContinuteProcess";
...
- (void)transferToNextStep{
    ...
    self.webServiceHelper = [[WebServiceHelper alloc] initWithUrl:strUrl method:@"testRunContinuteProcess" nameSpace:WEBSERVICE_NAMESPACE  parameters:params delegate:self];
...
- (void)saveToProcess{
    ...
    self.webServiceHelper = [[WebServiceHelper alloc] initWithUrl:strURL method:@"testRunSaveProcess" nameSpace:WEBSERVICE_NAMESPACE  parameters:params delegate:self];
...
- (void)processWebData:(NSData *)webData
{
    ...
    if (self.webServiceType == kWebService_SaveProcess) {
        WebDataParserHelper *webDataHelper = [[WebDataParserHelper alloc] initWithFieldName:@"testRunSaveProcessReturn" andWithJSONDelegate:self];
        ...
        WebDataParserHelper *webDataHelper = [[WebDataParserHelper alloc] initWithFieldName:@"testRunContinuteProcessReturn" andWithJSONDelegate:self];
