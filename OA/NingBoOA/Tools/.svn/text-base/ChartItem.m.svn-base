...
- (id)initWithValue:(float)val Name:(NSString*)title Color:(CGColorRef)aColor{
	if ((self = [super init])) {
		_value = val;
		_name = [[NSString alloc] initWithString:title];
		_color = aColor;
	}
	return self;
}
...
+(id)itemWithValue:(float)val Name:(NSString*)title Color:(CGColorRef)aColor{
	return [[self alloc] initWithValue:val Name:title Color:aColor ];
}

+ (NSMutableArray *)makeColorArray{
    NSMutableArray *colors = [[NSMutableArray alloc] initWithCapacity:15];
