...
{
	_unzFile = unzipOpen( (const char*)[zipFile UTF8String] );
	if( _unzFile )
	{
		unz_global_info  globalInfo = {0};
		if( unzipGetGlobalInfo(_unzFile, &globalInfo )==UNZ_OK )
		{
...
{
	BOOL success = YES;
	int ret = unzipGoToFirstFile( _unzFile );
	...
	do{
		if( [_password length]==0 )
			ret = unzipOpenCurrentFile( _unzFile );
		else
			ret = unzipOpenCurrentFilePassword( _unzFile, [_password cStringUsingEncoding:NSASCIIStringEncoding] );
		if( ret!=UNZ_OK )
		{
		...
		ret = unzipGetCurrentFileInfo(_unzFile, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:@"Error occurs while getting file info"];
			success = NO;
			unzipCloseCurrentFile( _unzFile );
			break;
		}
		char* filename = (char*) malloc( fileInfo.size_filename +1 );
		unzipGetCurrentFileInfo(_unzFile, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
		...
		if( [fman fileExistsAtPath:fullPath] && !isDirectory && !overwrite )
		{
			...
			{
				unzipCloseCurrentFile( _unzFile );
				ret = unzipGoToNextFile( _unzFile );
				continue;
			}
		}
		...
		while( fp )
		{
			read=unzipReadCurrentFile(_unzFile, buffer, 4096);
			if( read > 0 )
			{
...
-(BOOL) UnzipCloseFile
{
	_password = nil;
	if( _unzFile )
		return unzipClose( _unzFile )==UNZ_OK;
	return YES;
}
