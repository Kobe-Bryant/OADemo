...
- (void)alertView:(UIAlertView *)anAlertView clickedButtonAtIndex:(NSInteger)anIndex
{
    if (anIndex == 1)
    {
        ...
        [helper saveOneException:exception LoginUser:[usrInfo objectForKey:@"userId"] SoftInfo:getAppInfo() SBBH:[[UIDevice currentDevice] macaddress]];
...
- (void)handleException:(NSString*)exceptionStr
{
    ...
    UIAlertView *alert =
    [[UIAlertView alloc]
      initWithTitle:NSLocalizedString(@"系统异常", nil)
      message:@"发生异常错误，即将退出，是否将错误日志上传到服务器？\n"
      delegate:self
